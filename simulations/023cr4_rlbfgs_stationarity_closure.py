...
- `results/data/023cr4_n65_rlbfgs_checkpoint.npz`;
- on strict stationary success,
  `results/data/023cr4_strict_stationary_b7_n65.npz`.
...
This gate is GREEN_STRICT_STATIONARY_N65 only if all of the following hold:
    ...
    SAVED_FIELD_DIRECTIONAL_GRADIENT_CHECK=PASS
...
- A second directional derivative check is performed at the saved physical
  field itself.
...
--------------------
PROJECT_DERIVED_023CR4_RLBFGS_STATIONARITY_CLOSURE
"""
...
CHECKPOINT = ROOT / "results/data/023cr4_n65_rlbfgs_checkpoint.npz"
FINAL_ARTIFACT = ROOT / "results/data/023cr4_strict_stationary_b7_n65.npz"
...
def load_start(cr3, cr2) -> tuple[State, str]:
    ...
    allowed_source = source_tag in {"023CR3R_N65", "023CR4_RLBFGS", "023CR4_RLBFGS_EXIT"}
...
def saved_field_directional_check(cr3, state: State, g: np.ndarray):
    ...
    # Smooth deterministic random tangent direction; suppress the outer stencil
    # layers so the check probes the physical interior.
    ...
    v /= norm
    eps = 2.0e-7
    plus = cr3.exp_map_update(state.phi, v, eps)
    minus = cr3.exp_map_update(state.phi, v, -eps)
    Ep = cr3.high_order_energy_gradient(plus, state.dx, False)[0]
    Em = cr3.high_order_energy_gradient(minus, state.dx, False)[0]
    finite = (Ep - Em) / (2.0 * eps)
    analytic = cr3.tangent_inner(g, v, state.dx)
    rel = abs(finite - analytic) / max(abs(finite), abs(analytic), 1e-10)
    ok = rel <= 2.0e-5
    print(f"SAVED_FIELD_DIRECTIONAL_FINITE={finite:.15e}")
    print(f"SAVED_FIELD_DIRECTIONAL_ANALYTIC={analytic:.15e}")
    print(f"SAVED_FIELD_DIRECTIONAL_RELERR={rel:.15e}")
    print("SAVED_FIELD_DIRECTIONAL_GRADIENT_CHECK=" + ("PASS" if ok else "FAIL"))
    return ok
...
def rlbfgs(cr3, cr2, state: State):
    ...
    while accepted_this < MAX_ACCEPTED_THIS_RUN and not station:
        ...
        if state.accepted_total % CHECKPOINT_EVERY == 0 or station:
            save_checkpoint(state, E, rms, gmax, "023CR4_RLBFGS")
...
def main() -> None:
    print("=== 023CR4 — RLBFGS STATIONARITY CLOSURE ===", flush=True)
    ...
    save_checkpoint(state, E, rms, gmax, "023CR4_RLBFGS_EXIT")
    ...
    if not station:
        ...
        print("\n=== F — 023CR4 DECISION ===")
        print("023CR4_RLBFGS_STATIONARITY_CLOSURE=INCOMPLETE_CONTINUE_FROM_CHECKPOINT")
    ...
    cr3r.save_artifact(FINAL_ARTIFACT, state.phi, state.axis, state.dx, diag, payload, "023CR4_N65")
    print(f"STRICT_STATIONARY_FIELD_ARTIFACT={FINAL_ARTIFACT.relative_to(ROOT)}")

    print("\n=== G — 023CR4 DECISION ===")
    if physical:
        print("023CR4_RLBFGS_STATIONARITY_CLOSURE=GREEN_STRICT_STATIONARY_N65")
        ...
        print("023CR4_RLBFGS_STATIONARITY_CLOSURE=GREEN_STATIONARY_BUT_PHYSICAL_GATE_FAILED")
        print("UNRESTRICTED_CARTESIAN_STATIONARY_B7_FIELD=SUPPORTED_AT_N65")
    ...
    print("CLAIM_CLASSIFICATION=PROJECT_DERIVED_023CR4_RLBFGS_STATIONARITY_CLOSURE")
